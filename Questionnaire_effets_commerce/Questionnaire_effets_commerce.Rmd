...
mon_graph
girafe(print(mon_graph), width_svg = 8)
...
```{r fig.height=6}
viz_liensocial_frequence_confidence(rouen, "À la Métropole Rouen Normandie, 6 commerces sur 10 sont confidents de leurs clients hebdomadairement ou plus")
```
...
```{r}
viz_liensocial_associations(rouen, "À la Métropole Rouen Normandie, les <span style='color: #3182BD'>associations sociales</span> sont les <br>principales associations avec lesquelles les <br>commerçants s'associent") +
...
```{r}
viz_solidarites_redirection(rouen, "À la Métropole Rouen Normandie, le <span style='color: #3182BD'>social</span> est le principal type de <br>service vers lequel les commerçants orientent")
```
...
viz_viequartier_matrice_collab(rouen, 
                               "À la Métropole Rouen Normandie, les commerçants qui aident leurs \ncollègues ne collaborent pas \nnécessairement avec des associations")
```
...
```{r}
viz_environnement_conso_eauelec(rouen, "À la Métropole Rouen Normandie, la réduction des factures d'<span style='color: #3182BD'>électricité</span> <br>est la priorité des commerçants", 6.5, 43) +
    scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70), limits = c(0, 80))
```
...
table |> 
    ...
    theme_bw() +
    coord_flip() +
    labs(title = "À la Métropole Rouen Normandie, le recyclage des <span style='color: #3182BD'>piles électriques</span> est <br>une solution adoptée par une majorité de <br>commerçants recyclant",
         subtitle = "Solutions de recycalge ou récupération proposées par \nles commerçants",
         y = "Nombre de commerces") +
    scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
...
```{r}
viz_environnement_fournisseurs(rouen, "À la Métropole Rouen Normandie, 2 commerçants sur 10 recyclent \nleurs déchêts")
```
...
```{r}
viz_espacepublic_embellissement(rouen, "À la Métropole Rouen Normandie, les commerçants accordent une <br>attention particulière au <span style='color: #3182BD'>soin de leur vitrine</span>", 5.5, 50) +
    scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70), limits = c(0, 80))
```
...
stat_commune_finale |> 
    ...
    tab_spanner(label = md("**Métropole Rouen Normandie**"), columns = c(`Nombre de réponses `, `% `)) |> 
    # Style de la table
