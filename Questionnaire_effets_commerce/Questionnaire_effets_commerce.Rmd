...
table <- paris |> 
    ...
    select(`Lorsque vous êtes sollicité par quelqu'un en situation de précarité, quel type d'aide proposez-vous ? (Plusieurs réponses sont possibles)`,
           propose_qqchose, Libelle_acheminement, Code_commune_INSEE)
stat <- table |> 
...
stat_commune <- table |> 
    ...
    mutate(Catégorie = case_when(percent <= 25 ~ "Faible",
                                 percent > 25 & percent <= 50 ~ "Moyen",
                                 percent > 50 & percent <= 75 ~ "Élevé",
                                 percent > 75 ~ "Très élevé")) |> 
    arrange(desc(nb_reponses)) |> 
    select(Libelle_acheminement, nb_reponses, percent, Catégorie) |> 
    rename(Commune = Libelle_acheminement,
