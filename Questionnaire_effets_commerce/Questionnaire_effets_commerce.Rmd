...
mon_graph <- table |> 
    ggplot() +
      ...
      geom_label(aes(x = `Êtes-vous indépendant ?`, y = nb+15, 
                ...
                color = "#333333", size=5, check_overlap = T, fill = "white", label.size = NA) +
      theme_minimal() +
      custom_theme() +
...
mon_graph <- table |> 
    ...
    coord_flip()
mon_graph
girafe(print(mon_graph), width_svg = 7)
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
viz_liensocial_frequence_confidence <- function(donnees, title_name){
    ...
    table |> 
        ...
        labs(x = "", y = "Nombre de commerces", 
             ...
             title = stringr::str_wrap(title_name, width = 45)) +
        geom_label(aes(x = Var, y = n+0.1*max(n), label = ifelse(is.na(percent), NA, 
...
viz_liensocial_techniquestra <- function(donnees, title_name){
    ...
    labs(x = "Utilisation de techniques artisanales",
         y = "Nombre de commerces",
         title = str_wrap(title_name, width = 35),
...
mon_graph <- viz_solidarites_redirection(data, "Plus de 4 commerçants sur 10 ont déjà <br>réorienté quelqu’un vers un service <br>d’aide <span style='color: #3182BD'>sociale</span>")
mon_graph
saveWidget(girafe(print(mon_graph)), 
...
mon_graph <- viz_viequartier_matrice_collab(data, "L'entraide entre commerçants et \nl'inscription dans la vie associative \nlocale vont souvent de pair")
mon_graph
#girafe(print(mon_graph))
saveWidget(girafe(print(mon_graph)), 
...
mon_graph <- table |> 
  ...
  ggplot() + 
    ...
    labs(x = "", y = "Nombre de commerces", linetype = "",
         title = str_wrap("Un quart des commerçants redirige ses clients vers des commerces voisins", width = 40),
...
mon_graph <- table |> 
  ...
  ggplot() + 
    ...
    labs(x = "", y = "Nombre de commerces", linetype = "",
         title = str_wrap("Plus d'un tiers des commerçants réceptionne des colis pour ses voisins d'immeuble", width = 45),
...
mon_graph <- viz_environnement_conso_eauelec(data, "La réduction des factures d'<span style='color: #3182BD'>électricité</span> est la <br>priorité des commerçants", 6.5, 135)
mon_graph
#girafe(print(mon_graph))
saveWidget(girafe(print(mon_graph), width_svg = 8), 
...
word_data |> 
  ggplot(aes(label = word, size = freq)) +
      geom_text_wordcloud(color = rep_len(c('#afc7de','#9ecae1', "#3081bc"), nrow(word_data)), family = "Montserrat") +
      scale_size_area(max_size = 18) +
      labs(title = str_wrap("Autres équipements installés par les commerçants pour réduire leur consommation d'eau et d'électricité", width = 50)) +
      theme_minimal() +
...
viz_environnement_discu_envt <- function(donnees, title_name){
    ...
    table |> 
      ...
      ggplot() + 
        ...
        scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 35)) + #axis-text trop longs sur plusieurs lignes
        labs(x = "", y = "Nombre de commerces", linetype = "",
             title = str_wrap(title_name, width = 45),
...
viz_environnement_recyclage <- function(donnees, title_name,x_pos_text, y_pos_text){
    ...
    table |> 
        ...
        coord_flip() +
        labs(title = title_name,
             subtitle = "Solutions de recycalge ou récupération proposées par les commerçants",
             y = "Nombre de commerces") +
        scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
        scale_y_continuous(breaks = c(0, 50, 100, 150, 200), limits = c(0, 250)) + 
        custom_theme() +
...
mon_graph <- viz_environnement_recyclage(data, "Le recyclage des <span style='color: #3182BD'>cartons et tout-venant</span> est une <br>solution adoptée par une majorité de <br>commerçants recyclant", 8, 49.6)
mon_graph
#girafe(print(mon_graph))
saveWidget(girafe(print(mon_graph), width_svg = 8, height_svg = 7), 
...
graphique_apercu_portrait <- function(variable, data_geo, nom_geo, titre, nom_legende){
    ...
    ggplot(table) +
      ...
      scale_fill_manual(values = c("Oui" = "#3182bd", "Non" = "#9ecae1")) +
      scale_y_continuous(labels = scales::percent, limits = c(0,1)) +
      labs(title = str_wrap(titre, width = 42),
           fill = nom_legende,
           y = "Pourcentage des commerces") +
      theme_minimal() +
      custom_theme() +
...
```{r}
mon_graph <- viz_liensocial_techniquestra(paris, "À paris, plus de 7 commerçants sur 10 utilisent des techniques traditionnelles ou artisanales pour fabriquer leurs produits")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r}
mon_graph <- viz_solidarites_redirection(paris, "À Paris, plus de 4 commerçants sur 10 ont déjà <br>réorienté quelqu’un vers un service <br>d’aide <span style='color: #3182BD'>sociale</span>")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r fig.height=6}
mon_graph <- viz_environnement_conso_eauelec(paris, "À Paris, la réduction des factures d'<span style='color: #3182BD'>électricité</span> <br>est la priorité des commerçants", 6, 43) +
...
```{r}
viz_environnement_fournisseurs(paris, "À Paris, plus de 2 commerçants sur 10 \nchoisissent des fournisseurs écoresponsables")
```
...
```{r}
mon_graph <- viz_espacepublic_embellissement(paris, "À Paris, les commerçants accordent une <br>attention particulière au <span style='color: #3182BD'>soin de leur vitrine</span>", 5.5, 65) +
...
```{r}
mon_graph <- viz_espacepublic_detail_partimoine(paris, "À Paris, plus de 7 commerces sur 10 sont situés dans des bâtiments non classés", 0.2)
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r}
mon_graph <- viz_liensocial_techniquestra(stouen, "À St Ouen, 1 commerçant sur 2 utilise des techniques traditionnelles ou artisanales pour fabriquer ses produits")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r}
mon_graph <- viz_environnement_conso_eauelec(stouen, "À St Ouen, la réduction des factures <br>d'<span style='color: #3182BD'>électricité</span> est la seule action menée par <br>les commerçants", 1, 3) +
...
mon_graph <- rouen |> 
    ...
    ggplot()  +
      ...
      geom_col_interactive(fill = "#3182BD") +
  labs(y = "Nombre de commerces",
       title = "Catégorie des commerces de la Métropole \nRouen Normandie répondants") +
  coord_flip() +
  theme_minimal() +
  custom_theme() +
  theme(panel.grid.major.x = ggplot2::element_line(color = "#cbcbcb"),
...
```{r}
mon_graph <- viz_liensocial_associations(rouen, "À la Métropole Rouen Normandie, 4 commerçants sur 10 collaborent avec des <br><span style='color: #3182BD'>associations sociales</span>") +
...
```{r}
mon_graph <- viz_liensocial_techniquestra(rouen, "À la Métropole Rouen Normandie, 8 commerçants sur 10 utilisent des techniques traditionnelles ou artisanales pour fabriquer leurs produits")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r}
mon_graph <- viz_solidarites_redirection(rouen, "À la Métropole Rouen Normandie, plus de 4 commerçants sur 10 ont déjà <br>réorienté quelqu’un vers un service <br>d’aide <span style='color: #3182BD'>sociale</span>")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
mon_graph <- viz_viequartier_matrice_collab(rouen, 
                               "À la Métropole Rouen Normandie, l'entraide \nentre commerçants et l'inscription dans \nla vie associative locale ne vont pas \nnécessairement de pair")
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r}
mon_graph <- viz_environnement_conso_eauelec(rouen, "À la Métropole Rouen Normandie, la réduction <br>des factures d'<span style='color: #3182BD'>électricité</span> est la priorité des <br>commerçants", 6.5, 43) +
    scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70), limits = c(0, 80))
mon_graph
saveWidget(girafe(print(mon_graph), width_svg = 7.8, height_svg = 6), 
...
```{r}
viz_environnement_fournisseurs(rouen, "À la Métropole Rouen Normandie, 2 \ncommerçants sur 10 recyclent leurs déchêts")
```
...
```{r}
mon_graph <- cc_espacepublic(rouen, "     À la Métropole Rouen Normandie :")
```
