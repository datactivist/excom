...
codes_postaux_dg <- read_csv("codes_postaux_datagouv.csv") |>
...
# test <- data |> 
#   mutate(n_rep=n(), .by = Libelle_acheminement) |> 
#   distinct(Libelle_acheminement, population, n_rep) |> arrange(desc(population))
#table(data$categorie_population)
...
Les sous-populations basées sur le nombre d'habitants ont été créées à partir des [données de population de l'OFGL 2023](https://data.ofgl.fr/explore/dataset/ofgl-base-communes-consolidee/table/?disjunctive.reg_name&disjunctive.dep_name&disjunctive.epci_name&disjunctive.tranche_population&disjunctive.tranche_revenu_imposable_par_habitant&disjunctive.com_name&disjunctive.agregat&sort=exer&refine.exer=2023&refine.agregat=Achats+et+charges+externes) :
...
graph_questions_freq(table, ref_nettoyage_abords, "Ensemble des répondants", 
...
graph_comparaison_pop <- function(data, subtitle_name){
  data |> 
    ...
    labs(x = "", y = "Pourcentage des commerçants", linetype = "",
         subtitle = stringr::str_wrap(subtitle_name, width = 55),
...
### Discussions privées avec les clients{.tabset}
...
# On applique la fonction créée
graph_questions_freq(table, ref_discussions, "Ensemble des répondants", 
...
### Discussions informelles avec les clients{.tabset}
...
table <- data |>  
  ...
  mutate(`Avez-vous l'habitude de discuter avec vos clients en dehors du cadre strict de la vente ? (exemple : prendre des nouvelles personnelles)` = ifelse(`Avez-vous l'habitude de discuter avec vos clients en dehors du cadre strict de la vente ? (exemple : prendre des nouvelles personnelles)` == "Très rarement (moins d'une fois par semaine)", "Très rarement (moins d'une fois par mois)", `Avez-vous l'habitude de discuter avec vos clients en dehors du cadre strict de la vente ? (exemple : prendre des nouvelles personnelles)`),
         percent = n/sum(n)*100) |> 
  rename(Var = 1)
...
# On applique la fonction créée
graph_questions_freq(table, ref_discussions_informelles, "Ensemble des répondants", 
...
table <- data |>  
  ...
  na.omit() |>
  rename(Var = 1) |> 
  mutate(Var = ifelse(Var == "Très rarement (moins d'une fois par semaine)", "Très rarement (moins d'une fois par mois)", Var),
...
### Explications approfondies sur les produits{.tabset}
...
# On applique la fonction créée
graph_questions_freq(table, ref_explications_produits, "Ensemble des répondants", 
...
### Sollicitations par des personnes en précarité{.tabset}
...
# On applique la fonction créée
graph_questions_freq(table, ref_sollicitations_precaires, "Ensemble des répondants", 
...
### Redirections vers des aides extérieures{.tabset}
...
# On applique la fonction créée
graph_questions_freq(table, ref_redirections_aides_ext, "Ensemble des répondants", 
...
### Interventions dans des situations d'insécurité{.tabset}
...
table <- data |>  
  ...
  mutate(percent = n/sum(n)*100) |> 
  rename(Var = 1)
ref_situations_precaires <- table |> select(-n) |> rename(percent_ref = percent)
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Ensemble des répondants", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 10, "", "#3182bd") +
    theme(plot.caption = element_blank()) +
    geom_label(aes(x = Var, y = n+10, label = ifelse(is.na(percent), NA, paste0(round(percent, 0), "%"))),
               size = 5, fill = "white", label.size = NA)
```
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Ile-de-France", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 2, "", "#3182bd")
```
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Communes de moins de 1.000 habitants", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 1, "", "#DEEBF7")
```
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Communes de 1.000 à 9.999 habitants", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 2, "", "#9ECAE1")
```
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Communes de 10.000 à 99.999 habitants", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 2, "", "#3182BD")
```
...
# On applique la fonction créée
graph_questions_freq2(table, ref_situations_precaires, "Communes de 100.000 habitants et plus", 
                     "Répartition des commerces selon les interventions dans des situations d'insécurité", 3, "", "#0C4C8A")
```
...
# On applique la fonction créée
graph_comparaison_pop2(table, "Répartition des commerces selon les interventions dans des situations d'insécurité")
```




### Aide aux personnes accidentées ou souffrantes{.tabset}
...
# On applique la fonction créée
graph_questions_freq2(table, ref_aide_personnes, "Ensemble des répondants", 
...
### Situations d'insécurité des citoyens{.tabset}
...
table <- data |>  
  ...
  mutate(`L'espace public peut être le lieu où se déroule une série de situations portant attente à la sécurité des citoyens – harcèlement de rue, vol, agression physique, sexuelle ou verbale, etc. Suite à un de ces évènements, les personnes victimes rentrent dans des commerces pour se sentir en sécurité. À quelle fréquence avez-vous affaire à ce genre de situation ?` = str_squish(`L'espace public peut être le lieu où se déroule une série de situations portant attente à la sécurité des citoyens – harcèlement de rue, vol, agression physique, sexuelle ou verbale, etc. Suite à un de ces évènements, les personnes victimes rentrent dans des commerces pour se sentir en sécurité. À quelle fréquence avez-vous affaire à ce genre de situation ?`),
         percent = n/sum(n)*100) |> 
  rename(Var = 1)
...
# On applique la fonction créée
graph_questions_freq2(table, ref_insecurite_citoyens, "Ensemble des répondants", 
...
### Interpellation des services de la mairie{.tabset}
...
table <- data |>  
  ...
  mutate(`Vous arrive-t-il d'interpeller les services techniques de la mairie suite à du matériel défectueux ou une dégradation de l'espace public environnant (exemples : lampadaire défectueux ou tags) ?` = str_squish(`Vous arrive-t-il d'interpeller les services techniques de la mairie suite à du matériel défectueux ou une dégradation de l'espace public environnant (exemples : lampadaire défectueux ou tags) ?`),
         percent = n/sum(n)*100) |> 
  rename(Var = 1)
...
# On applique la fonction créée
graph_questions_freq(table, ref_interpellation_mairie, "Ensemble des répondants", 
