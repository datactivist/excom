...
graph_nettoyage(table, "Ensemble des répondants", 5, "") +
    theme(plot.caption = element_blank()) +
    geom_label(aes(x = Var, y = n+5, label = ifelse(is.na(percent), NA, paste0(round(percent, 0), "%"))),
               size = 5, fill = "white", label.size = NA)
```
...
# Graphique
graph_nettoyage(table, "Communes de moins de 1.000 habitants", 1, "")
```
...
# Graphique
graph_nettoyage(table, "Communes de 1.000 à 9.999 habitants", 1, "")
```
...
# Graphique
graph_nettoyage(table, "Communes de 10.000 à 99.999 habitants", 1, "")
```
...
# Graphique
graph_nettoyage(table, "Communes de 100.000 habitants et plus", 1, "")
```
...
table <- data |>  
  ...
  na.omit() |>
  mutate(percent = n/sum(n)*100, .by = categorie_population) |> 
  rename(Var = 1)

# Graphique
table |> 
    left_join(ref_nettoyage_abords, by = "Var") |> 
    mutate(n_ref = percent_ref/100 * sum(n[categorie_population == "élevée"])) |> 
    mutate(ecart = round((percent - percent_ref), 0),
           ecart = case_when(ecart >= 0 ~ paste0("+", ecart), .default = as.character(ecart))) |> 
    mutate(Var = factor(Var, levels = c("Jamais", "Très rarement (moins d'une fois par mois)", "Rarement (1 à 2 fois par mois)", "Ponctuellement (1 à 2 fois par semaine)", "Régulièrement (3 à 5 fois par semaine)", "Très régulièrement (plusieurs fois par jour)")),
           categorie_population = factor(categorie_population, levels = c("faible", "moyenne", "élevée"))) |> 
    ggplot() + 
    geom_bar(aes(y=n, x=Var, fill = categorie_population, group = categorie_population, alpha = Var != ""),
             position="dodge", stat="identity", width=.6) +
    geom_bar(aes(y=n_ref, x=Var, fill = categorie_population, group = categorie_population, linetype = "proportions de l'échantillon global"), 
             position="dodge", stat="identity", width=.6, size = 1, fill = NA, color = "#666666") +
    scale_fill_manual(values = c("faible" = "#DEEBF7", "moyenne" = "#9ECAE1", "élevée" = "#3182BD")) +
    scale_linetype_manual(values = c("proportions de l'échantillon global" = "dashed")) +
    scale_alpha_manual(values=c(1, .4)) +
    scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 25)) + #axis-text trop longs sur plusieurs lignes
    labs(x = "", y = "Nombre d'utilisateurs", linetype = "",
         subtitle = stringr::str_wrap("Répartition des commerces selon leur soin à nettoyer les abords extérieurs", width = 55),
         caption = "Les écarts de proportion avec l'échantillon global sont\nexprimés en points de pourcentage",
         title = stringr::str_wrap("Comparaison des communes selon leur population", width = 45)) +
    geom_label(aes(x = Var, y = n+1, fill = categorie_population, group = categorie_population, label = ifelse(is.na(ecart), NA, ecart)),
               size = 4, fill = "white", label.size = NA, position = position_dodge(width=.6), stat="identity", ) +
    theme_classic() +
    custom_theme() +
    theme(legend.position = "top", legend.box = "vertical", legend.box.just = "left",
          axis.title.x = element_blank()) +
    guides(alpha = "none", fill = guide_legend(title = "Population")) +
    coord_flip()
```
...
graph_discussions(table, "Ensemble des répondants", 5, "") +
    theme(plot.caption = element_blank()) +
    geom_label(aes(x = Var, y = n+5, label = ifelse(is.na(percent), NA, paste0(round(percent, 0), "%"))),
               size = 5, fill = "white", label.size = NA)
```
...
# On applique la fonction créée
graph_discussions(table, "Communes de moins de 1.000 habitants", 1, "")
```
...
# On applique la fonction créée
graph_discussions(table, "Communes de 1.000 à 9.999 habitants", 1, "")
```
...
# On applique la fonction créée
graph_discussions(table, "Communes de 10.000 à 99.999 habitants", 1, "")
```
...
# On applique la fonction créée
graph_discussions(table, "Communes de 100.000 habitants et plus", 1, "")
```
