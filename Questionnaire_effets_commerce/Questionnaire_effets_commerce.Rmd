...
viz_liensocial_techniquestra <- function(donnees, title_name){
    ...
    val2 |> 
      ...
      ggplot(aes(x="", y=n, fill=`Utilisez-vous des techniques traditionnelles et/ou artisanales pour fabriquer vos produits ?`,
                 tooltip = paste("Utilisation", `Utilisez-vous des techniques traditionnelles et/ou artisanales pour fabriquer vos produits ?`, ":", n, "commerces"))) +  
          geom_bar_interactive(stat="identity", width=1, color="white", size = 2) +
          coord_polar("y") +
          scale_fill_manual(values = c("Oui" = "#3182bd", "Non" = "#9ecae1")) +
...
viz_santesecu_frequence_accidents <- function(donnees, title_name, valeur_ressortir){
    ...
    graph_type_commerce_freq_agregee2(table, ref_data_freq_agregee, title_name, 
        ...
        geom_bar_interactive(aes(y=n, x=Var, alpha = Var != "",
                                 tooltip = paste(Var, ":", n, "commerces")),
                 position="dodge", stat="identity", width=.6, fill = "#3182bd") +
        theme(plot.caption = element_blank()) +
        geom_label(aes(x = Var, y = n+0.1*max(n), label = ifelse(is.na(percent), NA, paste0(round(percent, 0), "%"))),
...
mon_graph <- viz_espacepublic_detail_partimoine(data, "Près d'un commerce sur trois porte une attention particulière à l'entretien de sa façade")
mon_graph
#girafe(print(mon_graph))
saveWidget(girafe(print(mon_graph), width_svg = 7), 
...
```{r fig.height=6}
viz_liensocial_frequence_confidence(paris, "À Paris, près de 7 commerces sur 10 ont des discussions privées avec leurs clients au moins une fois par semaine")
```
...
```{r}
viz_solidarites_frequence_redirection(paris, "À Paris, plus d'un quart des commerçants dirige des clients vers des aides extérieures chaque semaine", "1 fois par semaine et plus")
```
...
```{r fig.height=6}
viz_liensocial_frequence_confidence(stouen, "À St Ouen, près de 2 commerçants sur 3 ont des discussions privées avec leurs clients au moins une fois par semaine")
```
...
```{r}
viz_solidarites_frequence_redirection(stouen, "À St Ouen, 2/3 des commerçants dirige des clients vers des aides extérieures chaque semaine", "1 fois par semaine et plus")
```
...
```{r fig.height=6}
viz_liensocial_frequence_confidence(rouen, "À la Métropole Rouen Normandie, 6 commerces sur 10 ont des discussions privées avec leurs clients au moins une fois par semaine")
```
...
```{r}
viz_liensocial_associations(rouen, "À la Métropole Rouen Normandie, les <br><span style='color: #3182BD'>associations sociales</span> sont les principales <br>associations avec lesquelles les <br>commerçants s'associent") +
...
```{r}
viz_liensocial_techniquestra(rouen, "À la Métropole Rouen Normandie, 8 commerçants sur 10 utilisent des techniques traditionnelles ou artisanales pour fabriquer leurs produits")
```
...
```{r}
viz_solidarites_frequence_nouvelles(rouen, "À la Métropole Rouen Normandie, près de 9 commerçants sur 10 prennent des nouvelles de leurs clients au moins une fois par semaine", "1 fois par semaine et plus")
```

```{r}
viz_solidarites_redirection(rouen, "À la Métropole Rouen Normandie, le <span style='color: #3182BD'>social</span> est <br>le principal type de service vers lequel les <br>commerçants orientent")
```

```{r}
viz_solidarites_frequence_redirection(rouen, "À la Métropole Rouen Normandie, 45% des commerçants dirigent des clients vers des aides extérieures chaque semaine", "1 fois par semaine et plus")
```
...
```{r fig.width=11}
viz_viequartier_events(rouen, "À la Métropole Rouen Normandie, 44% des commerçants participent à l'organisation d'évènements dans leur quartier")
```
...
```{r}
viz_santesecu_frequence_insecurite(rouen, "À la Métropole Rouen Normandie, près de 4 commerces sur 10 interviennent en cas d'insécurité au moins chaque mois", "1 fois par mois et plus")
```

```{r}
viz_santesecu_frequence_accidents(rouen, "À la Métropole Rouen Normandie, 22% des commerçants aident en cas d'accidents ou malaise au moins chaque mois", "1 fois par mois et plus")
```

```{r}
viz_santesecu_frequence_vols(rouen, "À la Métropole Rouen Normandie, près de 2 commerçants sur 10 sont confrontés à l'insécurité chaque mois", "1 fois par mois et plus")
```
...
```{r}
viz_environnement_conso_eauelec(rouen, "À la Métropole Rouen Normandie, la réduction <br>des factures d'<span style='color: #3182BD'>électricité</span> est la priorité des <br>commerçants", 6.5, 43) +
    scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70), limits = c(0, 80))
```

```{r}
viz_environnement_discu_envt(rouen, "À la Métropole Rouen Normandie, près de 4 commerçants sur 10 ont des discussions informelles avec ses clients sur l'environnement") +
...
```{r}
viz_espacepublic_embellissement(rouen, "À la Métropole Rouen Normandie, les <br>commerçants accordent une attention <br>particulière au <span style='color: #3182BD'>soin de leur vitrine</span>", 5.5, 50) +
    scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50, 60, 70), limits = c(0, 80))
```

```{r}
viz_espacepublic_frequence_appelmairie(rouen, "À la Métropole Rouen Normandie, 1 commerçant sur 2 appelle la mairie moins d'une fois par semaine", "Moins d'1 fois par semaine")
```

```{r}
viz_espacepublic_frequence_nettoyage(rouen, "À la Métropole Rouen Normandie, 6 commerçants sur 10 nettoyent les abords de leur commerce chaque semaine", "1 fois par semaine et plus")
```

```{r}
viz_espacepublic_patrimoine(rouen, "À la Métropole Rouen Normandie, 28% des commerces intérrogés se situent dans un bâtiment classé")
```

```{r}
viz_espacepublic_detail_partimoine(rouen, "À la Métropole Rouen Normandie, plus de 6 commerces sur 10 sont situés dans des bâtiments non classés")
```
