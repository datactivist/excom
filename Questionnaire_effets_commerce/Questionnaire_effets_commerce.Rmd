...
```{r}
# Fonction pour générer le graph
viz_environnement_conso_eauelec <- function(donnees, title_name){
    # Données
    table <- donnees |> 
        mutate(var = str_replace_all(`Avez-vous installé des équipements pour réduire la consommation d'eau ou d'électricité de votre commerce ?(Plusieurs réponses sont possibles)`, c("Equipements visant à réduire la consommation d'électricité \\(exemples : ampoules avec senseur de présence, multiprises avec timer\\)" = "Equipements visant à réduire la consommation d'électricité \\(exemples : ampoules avec senseur de présence multiprises avec timer\\)",  "Low tech \\(technologies ou machines simples à basse consommation\\) pour fabriquer mes produits \\(exemples : saponification à froid, feu de bois, marmite norvégienne\\)" = "Low tech\\(technologies ou machines simples à basse consommation\\) pour fabriquer les produits \\(exemples : saponification à froid feu de bois marmite norvégienne\\)"))) |> 
        mutate(var = str_split(var, ",")) |> 
...
viz_environnement_conso_eauelec(data, "La réduction des factures d'<span style='color: #3182BD'>électricité</span> est la <br>priorité des commerçants")
```
...
```{r fig.height=7}
# Fonction pour générer le graph
viz_environnement_recyclage <- function(donnees, title_name){
    # Données
    table <- donnees |> 
        mutate(Var = str_split(`Proposez-vous au sein de votre commerce des solutions de recyclage ?(Plusieurs réponses sont possibles)`, ",")) |> 
        unnest(Var) |> 
...
viz_environnement_recyclage(data, "Le recyclage des <span style='color: #3182BD'>cartons et tout-venant</span> est une <br>solution adoptée par une majorité de <br>commerçants recyclant")
```
...
viz_environnement_fournisseurs(data, "Près de 2 commerçants sur 10 font le choix de \nfournisseurs écoresponsables pour réduire \nleurs déchêts")
```
...
```{r}
# Fonction pour générer le graph
viz_espacepublic_embellissement <- function(donnees, title_name){
    # Données
    table_embellissement <- donnees |> 
        mutate(Var = str_replace_all(`Êtes-vous impliqué dans des initiatives d'embellissement de la rue ?`, 
                                     "Verdissement \\(exemples : jardinières, pots de fleurs\\)", "Verdissement")) |> 
...
viz_espacepublic_embellissement(data, "Les commerçants accordent une attention <br>particulière au <span style='color: #3182BD'>soin de leur vitrine</span>")
```
...
viz_espacepublic_detail_partimoine(data, "2/3 des commerces sont situés dans des batiments non classés")
```
...
```{r}
# Données
graphique_apercu_portrait <- function(variable, titre, nom_legende){
...
```{r}
viz_solidarites_frequence_redirection(paris, "À Paris, plus d'un quart des commerçants dirigent des clients vers des aides extérieures", "1 fois par semaine et plus")
```
...
```{r}
viz_environnement_conso_eauelec(paris, "À Paris, la réduction des factures d'<span style='color: #3182BD'>électricité</span> <br>est la priorité des commerçants")
```

```{r}
viz_environnement_discu_envt(paris, "À Paris, 3 commerçants sur 10 ont des discussions informelles avec ses clients sur l'environnement")
```

```{r}
viz_environnement_recyclage(paris, "À Paris, le recyclage des <span style='color: #3182BD'>cartons et tout-venant</span> est une <br>solution adoptée par une majorité de <br>commerçants recyclant")
```

```{r}
viz_environnement_fournisseurs(paris, "À Paris, plus de 2 commerçants sur 10 font le choix de \nfournisseurs écoresponsables pour réduire \nleurs déchêts")
```
...
```{r}
viz_espacepublic_embellissement(paris, "À Paris, les commerçants accordent une <br>attention particulière au <span style='color: #3182BD'>soin de leur vitrine</span>")
```
...
```{r eval=FALSE, include=FALSE}
graphique_apercu_portrait(variable = `Êtes-vous en contact direct avec des clients ?`,
                          titre = "Relation clientèle des commerces audoniens répondants, parmi l'ensemble des répondants",
                          nom_legende = "Contact direct avec des clients")
```
...
```{r}
viz_liensocial_associations(stouen, "À St Ouen, les <span style='color: #3182BD'>associations sociales</span> sont les <br>principales associations avec lesquelles les <br>commerçants s'associent")
```
...
```{r}
viz_solidarites_redirection(stouen, "À St Ouen, la <span style='color: #3182BD'>santé</span> est le principal type de <br>service vers lequel les commerçants orientent")
```
...
```{r fig.width=11}
viz_viequartier_events(stouen, "À St Ouen, 4 commerçants sur 10 participent à l'organisation d'évènements dans leur quartier")
```
...
```{r}
viz_environnement_conso_eauelec(stouen, "À St Ouen, la réduction des factures <br>d'<span style='color: #3182BD'>électricité</span> est la priorité des commerçants")
```
...
```{r}
viz_environnement_recyclage(stouen, "À St Ouen, le recyclage des <span style='color: #3182BD'>vêtements</span> est une <br>solution adoptée par une majorité de <br>commerçants recyclant")
```
...
```{r}
viz_espacepublic_embellissement(stouen, "À St Ouen, les commerçants accordent une <br>attention particulière au <span style='color: #3182BD'>soin de leur vitrine</span>")
```
...
```{r eval=FALSE, include=FALSE}
graphique_apercu_portrait(variable = `Êtes-vous en contact direct avec des clients ?`,
                          titre = "Relation clientèle des commerces rouennais répondants, parmi l'ensemble des répondants",
                          nom_legende = "Contact direct avec des clients")
```
...
```{r fig.height=6}
viz_liensocial_frequence_confidence(rouen, "À Rouen, près de 7 commerces sur 10 sont confidents de leurs clients hebdomadairement ou plus")
```

```{r}
viz_liensocial_frequence_expliproduits(rouen, 
                                       "À Rouen, 97% des commerçants expliquent leurs produits au moins une fois par semaine", 
                                       "1 fois par semaine et plus")
```

```{r}
viz_liensocial_associations(rouen, "À Rouen, les <span style='color: #3182BD'>associations de quartier</span> sont les <br>principales associations avec lesquelles les <br>commerçants s'associent")
```

```{r}
viz_liensocial_techniquestra(rouen, "À Rouen, plus de 7 commerçants sur 10 utilisent des techniques traditionnelles ou artisanales pour fabriquer leurs produits")
```
...
viz_solidarites_frequence_sollicitations(rouen, 
                                         "À Rouen, 1 commerce sur 4 est sollicité au moins une fois par semaine par des personnes précaires", 
                                         "1 fois par semaine et plus")
```
...
```{r}
viz_solidarites_redirection(rouen, "À Rouen, le <span style='color: #3182BD'>social</span> est le principal type de service <br>vers lequel les commerçants orientent")
```

```{r}
viz_solidarites_frequence_redirection(rouen, "À Rouen, plus d'un quart des commerçants dirigent des clients vers des aides extérieures", "1 fois par semaine et plus")
```
...
```{r fig.width=11}
viz_viequartier_events(rouen, "À Rouen, 3 commerçants sur 10 participent à l'organisation d'évènements dans leur quartier")
```
...
```{r}
viz_santesecu_frequence_insecurite(rouen, "À Rouen, 2 commerces sur 10 interviennent en cas d'insécurité au moins chaque mois", "1 fois par mois et plus")
```

```{r}
viz_santesecu_frequence_accidents(rouen, "À Rouen, près d'un quart des commerçants aide en cas d'accidents ou malaise au moins chaque mois", "1 fois par mois et plus")
```

```{r}
viz_santesecu_frequence_vols(rouen, "À Rouen, plus d'un commerçant sur 10 est confronté à l'insécurité chaque mois", "1 fois par mois et plus")
```
...
```{r}
viz_environnement_conso_eauelec(rouen, "À Rouen, la réduction des factures d'<span style='color: #3182BD'>électricité</span> <br>est la priorité des commerçants")
```

```{r}
viz_environnement_discu_envt(rouen, "À Rouen, 3 commerçants sur 10 ont des discussions informelles avec ses clients sur l'environnement")
```

```{r}
viz_environnement_recyclage(rouen, "À Rouen, le recyclage des <span style='color: #3182BD'>cartons et tout-venant</span> est une <br>solution adoptée par une majorité de <br>commerçants recyclant")
```

```{r}
viz_environnement_fournisseurs(rouen, "À Rouen, plus de 2 commerçants sur 10 font le choix de \nfournisseurs écoresponsables pour réduire \nleurs déchêts")
```
...
```{r}
viz_espacepublic_embellissement(rouen, "À Rouen, les commerçants accordent une <br>attention particulière au <span style='color: #3182BD'>soin de leur vitrine</span>")
```
...
```{r}
viz_espacepublic_frequence_nettoyage(rouen, "À Rouen, 7 commerçants sur 10 nettoyent les abords de leur commerce chaque semaine", "1 fois par semaine et plus")
```

```{r}
viz_espacepublic_patrimoine(rouen, "À Rouen, 1 commerce sur 10 se situe dans un bâtiment classé")
```

```{r}
viz_espacepublic_detail_partimoine(rouen, "À Rouen, plus de 7 commerces sur 10 sont situés dans des bâtiments non classés")
```
...
calcul_indicateur_territoire <- function(territoire, territoire_nom){
    ...
    table <- data |> 
        ...
        mutate(variable_indic = ifelse(`Certaines personnes rentrent dans un magasin pour acheter un produit, mais aussi pour se sentir moins seules. Il se peut que certains de vos clients vous confient leurs problématiques personnelles. A quelle fréquence échangez-vous sur ces sujets privés ?` == "Jamais", "0", "1"))
        # Calcul de l'indicateur
    fonction_stat_commune(table,
                          territoire = territoire,
                          Externalité_texte = "Solidarités", 
                          Indicateur_texte = "% de commerces a qui se confient les clients sur des sujets privés",
    ...
    table <- data |> 
        ...
        mutate(variable_indic = ifelse(`Avez-vous l'habitude de discuter avec vos clients en dehors du cadre strict de la vente ? (exemple : prendre des nouvelles personnelles)` == "Jamais", "0", "1"))
        # Calcul de l'indicateur
    fonction_stat_commune(table,
                          territoire = territoire,
                          Externalité_texte = "Lien social", 
                          Indicateur_texte = "% de commerces ayant des discussions informelles avec leurs clients",
    ...
    table <- data |> 
        ...
        mutate(variable_indic = ifelse(`À quelle fréquence donnez-vous des explications approfondies sur vos produits à vos clients ? Cela peut aussi concerner des démonstrations.` == "Jamais", "0", "1"))
        # Calcul de l'indicateur
    fonction_stat_commune(table,
                          territoire = territoire,
                          Externalité_texte = "Lien social", 
                          Indicateur_texte = "% de commerces donnant des explications approfondies sur les produits",
    ...
    table <- data |> 
        ...
        mutate(variable_indic = ifelse(`L'entraide entre commerçants permet de faire face à des difficultés et constitue un réseau d'échange de services. Quels services avez-vous l'habitude de rendre à vos collègues commerçants ? (Plusieurs réponses sont possibles)` %in% c("Je ne connais pas vraiment les autres commerçants de ma rue/quartier", "Je connais les autres commerçants mais je n'ai pas l'occasion de rendre ces services"), "0", "1"))
        # Calcul de l'indicateur
    fonction_stat_commune(table,
                          territoire = territoire,
                          Externalité_texte = "Vie de quartier", 
                          Indicateur_texte = "% de commerces rendant des services à leurs collègues commerçants",
...
stat_commune_finale |> 
    ...
    # Stype de la table
    tab_style(style = list(cell_text(weight = "lighter")), locations = cells_body(columns = Indicateur)) |>  
